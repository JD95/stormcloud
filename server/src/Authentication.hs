...
newtype ServerSecret = ServerSecret ByteString

sessionLength = 600 -- seconds

genToken :: ServerSecret -- ^ Random bytes unique to each bootup
...
genSession :: Text -- ^ The google token string from client
...
genSession idToken s = do
  ...
  fmap join . for c' $ \c -> for t $ \token -> do
    user' <- runSQL . getBy . UserEmailAddress $ email token
    (userId, user) <- case user' of      
      Nothing -> do
...
logout = do
  token <- cookie "auth"
  flip (maybe (pure ())) token $ \t ->
    runSQL $ deleteWhere [SessionToken ==. t]
